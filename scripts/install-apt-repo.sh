...
if command -v curl >/dev/null 2>&1; then
  ...
  echo "CSV Anonymizer repository setup requires curl or wget." >&2
  exit 1
fi
...
require_expected_keyring_fingerprint() {
  ...
  if ! GNUPGHOME="$gnupg_home" gpg --batch --import-options show-only --with-colons --import "$signing_key_file" >"$keyring_fingerprints_file" 2>/dev/null; then
    echo "Could not inspect downloaded CSV Anonymizer setup signing keyring." >&2
    exit 1
  fi
  ...
  case "$keyring_validation" in
    OK)
      ;;
    NO_PRIMARY_KEY)
      echo "Downloaded CSV Anonymizer setup signing keyring did not contain a primary key." >&2
      exit 1
      ;;
    MISSING_PINNED_SIGNER)
      echo "Downloaded CSV Anonymizer setup signing keyring did not contain the pinned signer fingerprint." >&2
    ...
    UNEXPECTED_PRIMARY_KEY*)
      ...
      echo "Downloaded CSV Anonymizer setup signing keyring contained an unexpected primary key." >&2
      echo "Expected signer:    $expected_signing_fingerprint" >&2
      echo "Unexpected primary: $unexpected_primary_key" >&2
      exit 1
      ;;
    MULTIPLE_ACCEPTED_PRIMARY_KEYS)
      echo "Downloaded CSV Anonymizer setup signing keyring contained multiple primary keys for the pinned signer." >&2
      echo "Expected signer: $expected_signing_fingerprint" >&2
      exit 1
      ;;
    *)
      echo "Could not validate downloaded CSV Anonymizer setup signing keyring." >&2
      exit 1
      ;;
  esac
}
...
verify_sha256_sidecar_signature() {
  ...
  if ! GNUPGHOME="$gnupg_home" gpg --batch --status-fd 3 --verify "$setup_sha256_sig_file" "$setup_sha256_file" 3>"$signature_status_file" >/dev/null 2>&1; then
    echo "CSV Anonymizer setup SHA256 signature verification failed." >&2
    exit 1
  fi
  ...
  if [ "$validsig_matches" = "NO_VALIDSIG" ]; then
    echo "CSV Anonymizer setup SHA256 signature did not produce a VALIDSIG fingerprint." >&2
    exit 1
  fi

  if [ "$validsig_matches" != "OK" ]; then
    echo "CSV Anonymizer setup SHA256 signature was made by an unexpected signer." >&2
...
authenticate_sha256_sidecar() {
  if ! command -v gpg >/dev/null 2>&1; then
    echo "Missing gpg. Install GnuPG or provide CSV_ANONYMIZER_REPOSITORY_SETUP_SHA256 from a trusted source." >&2
    exit 1
  fi
  if [ -z "$expected_signing_fingerprint" ]; then
    echo "Missing pinned CSV Anonymizer setup signing key fingerprint." >&2
    echo "Use the release-published installer, or set CSV_ANONYMIZER_REPOSITORY_SETUP_SIGNING_KEY_FINGERPRINT from a trusted source." >&2
    exit 1
  fi

  printf 'Downloading CSV Anonymizer repository setup SHA256 sidecar and signature...\n'
  ...
  printf 'Authenticated CSV Anonymizer repository setup SHA256 signature with key %s.\n' "$expected_signing_fingerprint"
}

tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/csv-anonymizer-repository-setup.XXXXXX")"
setup_deb="$tmp_dir/csv-anonymizer-repository-setup_1.0_all.deb"
setup_sha256_file="$tmp_dir/csv-anonymizer-repository-setup_1.0_all.deb.sha256"
setup_sha256_sig_file="$tmp_dir/csv-anonymizer-repository-setup_1.0_all.deb.sha256.asc"
signing_key_file="$tmp_dir/csv-anonymizer-archive-keyring.pgp"
gnupg_home="$tmp_dir/gnupg"
mkdir -m 700 "$gnupg_home"

printf 'Downloading CSV Anonymizer repository setup package...\n'
...
if [ -z "$expected_sha256" ]; then
  echo "Could not resolve an authenticated SHA256 for CSV Anonymizer repository setup package." >&2
  echo "Use the signed sidecar path, or set CSV_ANONYMIZER_REPOSITORY_SETUP_SHA256 from a trusted source." >&2
  exit 1
fi

actual_sha256="$(calculate_sha256 "$setup_deb")"
if [ "$actual_sha256" != "$expected_sha256" ]; then
  echo "Checksum verification failed for CSV Anonymizer repository setup package." >&2
...
printf '  sudo apt install csv-anonymizer\n'
