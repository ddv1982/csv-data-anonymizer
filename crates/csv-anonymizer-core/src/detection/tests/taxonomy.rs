...
#[test]
fn multilingual_detector_evaluation_fixture_covers_claimed_languages() {
    let fixtures = [
        (
            "en",
            "phone",
            vec!["+1 415 555 0100", "+1 212 555 0101"],
            DataType::Phone,
        ),
        (
            "nl",
            "telefoon",
            vec!["+31 6 12345678", "+31 20 123 4567"],
            DataType::Phone,
        ),
        (
            "de",
            "geburtsdatum",
            vec!["1980-01-02", "1991-03-04"],
            DataType::Timestamp,
        ),
        (
            "fr",
            "adresse",
            vec!["12 Rue de Rivoli", "5 Avenue Victor Hugo"],
            DataType::Address,
        ),
        (
            "es",
        ...
        (
            "ja",
            "電話番号",
            vec!["+81 90 1234 5678", "+81 80 2345 6789"],
            DataType::Phone,
        ),
    ];
    ...
    for (language, header, values, expected) in fixtures {
        let result = detect_column_type_with_name(header, &strings(&values));
        assert_eq!(
            result.data_type, expected,
            "language {language}, header {header}"
        );
        assert_ne!(
            result.confidence,
            Confidence::Low,
