...
fn transforms_csv_text_with_existing_csv_rules() {
    ...
    let analysis = analyze_paste_data(
        PasteAnalyzeParams {
    ...
    let result = transform_paste_data(
...
fn plain_text_candidate_replay_rejects_stale_source_and_requires_selected_column() {
    ...
    let unchanged = replay_paste_text_candidate_evidence(
        &base,
        &snapshot,
    ...
    stale.content.push('!');
    assert!(
        replay_paste_text_candidate_evidence(
            &stale,
            &snapshot,
            &[candidate_id],
            crate::TransformRuntime::default()
...
fn direct_input_preview_classifies_on_the_whole_paste_not_the_display_window() {
    ...
    let preview = preview_paste_data(
        PastePreviewParams {
            content,
            format: PasteDataFormat::Csv,
            columns: vec![0],
...
fn generates_quick_values_without_user_input() {
    let result = generate_quick_values(
        QuickGenerateParams {
...
fn generates_type_shaped_quick_values() {
    let uuid_result = generate_quick_values(
        QuickGenerateParams {
    ...
    let ip_result = generate_quick_values(
        QuickGenerateParams {
    ...
    let phone_result = generate_quick_values(
        QuickGenerateParams {
    ...
    let name_result = generate_quick_values(
        QuickGenerateParams {
    ...
    let timestamp_result = generate_quick_values(
        QuickGenerateParams {
...
fn paste_tokenization_repeats_across_explicit_run_contexts_without_serializing_the_key() {
    ...
    let first = transform_paste_data(
        input.clone(),
        crate::TransformRuntime {
            provider: None,
            tokenization_key: Some(&first_key),
        },
    )
    .unwrap();
    let second = transform_paste_data(
        input,
        crate::TransformRuntime {
            provider: None,
            tokenization_key: Some(&second_key),
        },
    )
    .unwrap();
...
fn quick_generation_rejects_input_only_strategies() {
    let error = generate_quick_values(
        QuickGenerateParams {
            data_type: DataType::Email,
...
fn paste_transform_reuses_preview_smart_replacements_and_generates_missing_values() {
  ...
  {"name":"Katherine Johnson"}
    ...
    let result = transform_paste_data(
        ...
        crate::TransformRuntime {
            provider: Some(&mut transform_provider),
            tokenization_key: None,
        },
    )
    .unwrap();
